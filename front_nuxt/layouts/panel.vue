            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-white">
              ...
              <div class="h-0 flex-1 overflow-y-auto pt-5 pb-4">
                <div class="flex flex-shrink-0 items-center px-4">
                  <img class="h-10 w-auto" src="/logo-blue.svg" alt="Admeal" />
                </div>
              ...
              <div class="flex flex-shrink-0 border-t border-gray-200 p-4 bg-gray-100" v-if="user">
                <NuxtLink to="/profile" class="group block flex-shrink-0 w-full">
                  <div class="flex items-center justify-between w-full">
                    <div class="ml-3">
                      <p class="text-base font-medium text-gray-700 group-hover:text-gray-900">{{user.username}}</p>
...
watch(route, (to) => {
  console.log(to.path);
  if(navigation.value.map((nav) => nav.to).includes(to.path)) {
    const old = navigation.value.find((nav) => nav.current);
    if(old) {
      old.current = false;
    }
    const current = navigation.value.find(nav => nav.to.includes(to.path));
    if(current) {
